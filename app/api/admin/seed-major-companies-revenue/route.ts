/**
 * Admin endpoint to seed revenue data for major Romanian companies
  ...
  try {
    ...
    for (const [cui, data] of Object.entries(revenueData.companies)) {
      try {
        ...
        if (updated.count > 0) {
          results.updated++;
          console.log(`[seed-major-companies-revenue] Updated ${data.name} (${cui}): ${data.revenue.toLocaleString()} RON revenue`);
        } else {
          console.warn(`[seed-major-companies-revenue] Company not found: ${cui}`);
          results.notFound++;
        }
        ...
        console.error(`[seed-major-companies-revenue] Error updating ${cui}:`, error);
        Sentry.captureException(error, {
          tags: { component: "seed-major-companies-revenue", cui },
        });
      }
    }
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error("[seed-major-companies-revenue] Fatal error:", error);
    Sentry.captureException(error);
