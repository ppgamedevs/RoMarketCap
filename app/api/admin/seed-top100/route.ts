/**
 * PROMPT 63: Top 100 Romania Seed Loader
 * 
 * One-click ingestion of curated top Romanian companies with real CUIs.
 * This provides immediate quality data for the market view.
 */
...
export async function POST() {
  try {
    ...
    // Read seed file
    const seedPath = path.join(process.cwd(), "data", "seeds", "top100-romania.json");
    
    if (!fs.existsSync(seedPath)) {
      return NextResponse.json({
        ok: false,
        error: "Seed file not found at data/seeds/top100-romania.json",
      }, { status: 404 });
    }
      ...
      try {
        ...
        if (existing) {
          ...
          await applyPostIngestionHooks(existing.id).catch((err) => {
            console.error(`[seed-top100] Post-hooks failed for ${normalizedCui}:`, err);
          });
        } else {
          // Create new company
    ...
    console.error("[admin/seed-top100] Error:", error);
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : "Unknown error",
    }, { status: 500 });
