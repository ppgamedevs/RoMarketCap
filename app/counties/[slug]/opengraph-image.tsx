...
export default async function OGImage({ params }: Props) {
  ...
  const count = await prisma.company.count({ where: { isPublic: true, visibilityStatus: "PUBLIC", countySlug: slug } });
  const label = countyLabel(slug, "ro");
    ...
    (
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 14, opacity: 0.7 }}>County</div>
        </div>
